...
MYDLL_API HashMap HM_Init(EqualFunc ef, HashFunc hf, int size) {
    ...
    result.equalFunc = ef;
    result.hashFunc = hf;
    result.hashTable = calloc(size, sizeof(HashList));
    result.hashSize = size;
    return result;
}
...
MYDLL_API cpvoid HM_Get(const HashMap *self, cpvoid key) {
    uint32_t hash = (self->hashFunc(key)) % self->hashSize;
    HashList *hNode = self->hashTable + hash;
    //if there is no elements with such hash, we can't compare keys
    if (hNode->next == NULL) {
        return NULL;
    }

    //while there are non zero nodes with wrong keys
    do {
        hNode = hNode->next;
    }while(hNode->next != NULL && !self->equalFunc(hNode->key, key));

    //if we reached correct key
    if (self->equalFunc(hNode->key, key))
        return hNode->value;
    else
        return NULL;
}
...
MYDLL_API void HM_Set(HashMap *self, cpvoid key, cpvoid value) {
    ...
    //if there is no elements with such hash, we can't compare keys
    if (hNode->next == NULL) {
        hNode->next = calloc(1, sizeof(HashList));
        hNode = hNode->next;
        hNode->key = key;
        hNode->value = value;
        return;
    ...
    //otherwise, start traversing hash list
    do{
        hNode = hNode->next;
    }while (hNode->next != NULL && !self->equalFunc(hNode->key, key));
    ...
    if (self->equalFunc(hNode->key, key))
        ...
        //we didn't find they key -> let's create new node
    else {
        hNode->next = calloc(1, sizeof(HashList));
        hNode = hNode->next;
        hNode->key = key;
