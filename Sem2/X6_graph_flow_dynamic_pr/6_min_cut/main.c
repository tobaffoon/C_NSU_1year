...
void Mincut_push(Mincut *mc, int id){
	if(mc->cap == 0) {
		mc->cap = 2;
		mc->cut = malloc(mc->cap * sizeof(int));
	}
	else if(mc->n == mc->cap) {
		mc->cap *= 2;
		mc->cut = (int *) realloc(mc->cut, sizeof(int) * mc->cap);
	}
	mc->cut[mc->n++] = id;
}
...
int main() {
	...
	int start, end;
	fscanf(fi, "%d %d\n %d %d", &(graph.v_num), &(graph.e_num), &start, &end);
	...
	long long max_flow = Max_flow(graph, start - 1, end - 1);
	Mincut mincut = {0,0,0};
	for(int i = 0; i < graph.e_num * 2; i++) {
		if(graph.vertices[graph.edges[i].from].BFS_depth != UNREACHABLE && graph.vertices[graph.edges[i].to].BFS_depth == UNREACHABLE) {
			Mincut_push(&mincut, i);
		}
	}
	fprintf(fo, "%lld %d", max_flow, mincut.n);
	for(int i = 0; i < mincut.n; i++) {
		fprintf(fo, "\n%d %d", graph.edges[mincut.cut[i]].from + 1, graph.edges[mincut.cut[i]].to + 1);
	}
	if(mincut.cap!=0) {
		free(mincut.cut);
	}
